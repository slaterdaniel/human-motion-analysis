...
def save_video(file, worst_frame, worst_length, best_frame, best_length, raw_data, user_predictions, scored_data, window_size=9):
    ...
    for i in range(length + (offset * 2)):
        ...
        frame_scores = scored_data[:, curr_frame - boarder]
        score = round(np.sum(frame_scores ** 2), 1)
        biggest_mistake = np.argmax(frame_scores)

        error_value = round(raw_data[biggest_mistake, curr_frame - boarder], 2)
        reference_value = round(raw_data[biggest_mistake, int(best_start_frame + i) - boarder], 2)

        cv2.putText(read_frame,
                    f"Phase: {PHASE_STRINGS[user_predictions[curr_frame - boarder]]}",
                    (int(width * .7), int(height * .45)), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2, cv2.LINE_AA)

        cv2.putText(read_frame,
                    f"Score: {score}",
                    (int(width * .7), int(height * .5)), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2, cv2.LINE_AA)

        cv2.putText(read_frame,
                    f"{FEATURE_STRINGS[biggest_mistake]}:",
                    (int(width * .7), int(height * .6)), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

        cv2.putText(read_frame,
                    f"{'Error:':<8} {error_value:<8} Frame: {curr_frame}",
                    (int(width * .7), int(height * .65)), cv2.FONT_HERSHEY_SIMPLEX, 1, (127,127,255), 2, cv2.LINE_AA)

        cv2.putText(read_frame,
                    f"{'Ref:':<8} {reference_value:<8} Frame: {int(best_start_frame + i)}",
                    (int(width * .7), int(height * .7)), cv2.FONT_HERSHEY_SIMPLEX, 1, (127, 255, 127), 2, cv2.LINE_AA)

        cv2.putText(read_frame,
                    f"{'% Error:':<8} {round((error_value - reference_value) * 100 / (reference_value + 1e-8), 2)}%",
                    (int(width * .7), int(height * .75)), cv2.FONT_HERSHEY_SIMPLEX, 1, (127, 255, 255), 2, cv2.LINE_AA)
