...
BOE1 = "../../boeTread_pt1.mp4"
BOE2 = "../../boeTread_pt2.mp4"
...
def main():
        ...
        if results.pose_landmarks:
            ...
            for i, lm in enumerate(current_pose):
                if i in valid_landmarks:
                    ...
                    x_pixel = int((x / 3 + 0.5) * width)
                    y_pixel = int((y / 3 + 0.5) * height)
                    cv2.circle(canvas, (x_pixel, y_pixel), 5, (255, 255, 255), -1)
    ...
    if mode == '1':
        np.save('../../raw_data.npy', data[border: -border - 1])
    elif mode == '2':
        np.save('../../user_raw_data.npy', data[border: -border - 1])
    print(data[border: -border-1].shape)
    inputs = []
    ...
    if  mode == '1':
        if   video == BOE1:
            np.save('../../boeTreadpt1_capture.npy', inputs)
        elif video == BOE2:
            np.save('../../boeTreadpt2_capture.npy', inputs)

    elif mode == '2':
        np.save('../../user_capture.npy', inputs)
